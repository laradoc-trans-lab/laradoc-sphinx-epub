...
class TorchlightHtmlFormatter(HtmlFormatter):
    def __init__(self, **options):
        ...
        print("[DEBUG] TorchlightHtmlFormatter __init__ called!")
        self.in_add_block = False
        self.in_remove_block = False
        self.in_remove_block = False

    def wrap(self, source):
        print("[DEBUG] TorchlightHtmlFormatter wrap() method called!")
        ...
        for type, value in super().wrap(source):
            if type == 1: # This type typically indicates a line of code
                ...
                if tag_match:
                    ...
                    print(f"[DEBUG] Found tag: {tag_match.group(0)}. Type: {tag_type}, Subtype: {tag_subtype}")
                    print(f"[DEBUG] Before replacement (group 0): '{tag_match.group(0)}'")
                    print(f"[DEBUG] Before replacement (processed_value): '{processed_value.strip()}'")
                    ...
                    print(f"[DEBUG] After replacement (processed_value): '{processed_value.strip()}'")
                    ...
                    print(f"[DEBUG] Tag removed. Processed value after removal: {processed_value.strip()}")
                
                # --- Apply highlighting based on current state (for ranges) ---
                elif self.in_add_block:
                    line_highlight_class = "hll"
                    print(f"[DEBUG] Applying hll due to active add block.")
                elif self.in_remove_block:
                    line_highlight_class = "dll"
                    print(f"[DEBUG] Applying dll due to active remove block.")
                ...
                if line_highlight_class:
                    ...
                    print(f"[DEBUG] Wrapped line with {line_highlight_class}. Final HTML: {processed_value.strip()}")
                else:
...
def setup(app):
    print("[DEBUG] Torchlight Sphinx extension loaded!")
